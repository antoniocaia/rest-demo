...
public class EmployeeModelAssembler implements RepresentationModelAssembler<Employee, EntityModel<Employee>> {
	...
	public EntityModel<Employee> toModel(Employee employee) {
		...
		return EntityModel.of( employee ); // TODO There is a bugs here :(
			//linkTo(methodOn(EmployeeController.class).getById(employee.getId())).withSelfRel() );	// Links the method one(), and define its type as self link, or self reference
			//linkTo(methodOn(EmployeeController.class).getAll()).withRel("employees"));			// Links the method all(), and call it "employees"
	}	
}